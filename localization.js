...
function tr(v){if(typeof v!=="string")return v;const t=v.trim();const override=coverOverrides[lang]?.get(t);if(override!==undefined)return v.replace(t,override);if(lang!=="es")return v;if(exact.has(t))return v.replace(t,exact.get(t));let o=v;
...
new MutationObserver(ms=>{for(const m of ms){for(const n of m.addedNodes){if(n.nodeType===1)walk(n);else if(n.nodeType===3){const x=tr(n.nodeValue);if(x!==n.nodeValue)n.nodeValue=x}}if(m.type==="characterData"){const x=tr(m.target.nodeValue);if(x!==m.target.nodeValue)m.target.nodeValue=x}}}).observe(document.documentElement,{subtree:true,childList:true,characterData:true});
})();